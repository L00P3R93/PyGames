"""General Utilities"""
import time, pygame, pygame.gfxdraw


# Called when drawing rounded rect
...
# Draws solid rounded rect
def _filled_rounded_rect(surf, color, rect, r):
    for x, y in [(rect[0] + r, rect[1] + r), (rect[0] + rect[2] - r - 1, rect[1] + r),
                 (rect[0] + r, rect[1] + rect[3] - r - 1), (rect[0] + rect[2] - r - 1, rect[1] + rect[3] - r - 1)]:
...
# Decorator function to check execution time of a function [Testing]
def timeit(func):
