...
def scan():
    for i in range(20):
        pth = os.path.join("res", "saveGames", "game" + str(i) + ".txt")
...
def delGame(gameId):
    name = os.path.join("res", "saveGames", "game" + str(gameId) + ".txt")
...
def loadGame(gameId):
    ...
    if os.path.exists(name):
        ...
        if lines[0].strip() == "mulit":
            temp = list(map(int, lines[3].strip().split()))
            if len(temp) == 0:
                return "mulit", None, None, lines[2]
            elif len(temp) == 1:
                return "mulit", temp[0], None, lines[2]
            else:
...
def showScreen(win, pg, scanned):
    ...
    for cnt, i in enumerate(scanned):
        if cnt // 5 == pg:
            ...
            pygame.draw.line(win, (255, 255, 255), (118, num + 5), (226, num + 45), 2)
            win.blit(LOADGAME.TYPEHEAD, (122, num + 2))
            win.blit(LOADGAME.TYP[i[1]], (122, num + 23))
            pygame.draw.line(win, (255, 255, 255), (226, num + 5), (226, num + 45), 2)
