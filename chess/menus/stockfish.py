...
# Shows a popup on screen whether stockfish is configured or not
def install(win, pth):
...
# Shows a prompt if user decides to quit before configuring stockfish
def prompt(win):
...
# Manages stockfish install guide
def guideMain(win):
    OS = platform.system()
    pg = True
    clock = pygame.time.Clock()
