"""
Module meant to provide a high-level TextBox in Pygame
Designed by https://github.com/ankith26/My-PyChess/commits?author=ankith26
to be a fully KEYBOARD and MOUSE INTERACTIVE
"""
import os, pygame
...
class TextBox:
    ...
    def draw(self, win):
        ...
        pygame.draw.rect(self.surf, (255, 255, 255), (3, 3, self.RECT[2] - 6, self.RECT[3] - 6))
        cursorpos = self.getLen([0, self.cursor])
        rendered = pygame.Surface((self.getLen() + 2, self.RECT[3] - 8))
        rendered.fill((255, 255, 255))

        if self.selected is not None:
            selrect = (self.getLen([0, self.selected[0]]), 2, self.getLen(self.selected), self.RECT[3] - 12)
            pygame.draw.rect(rendered, (128, 220, 255), selrect)
        rendered.blit(self.renderText(), (0, 0))

        if self.active:
            pygame.draw.rect(self.surf, (0, 0, 255), (2, 2, self.RECT[2] - 5, self.RECT[3] - 5), 2)
            if self.visible:
                pygame.draw.line(rendered, (0, 0, 0), (cursorpos, 2), (cursorpos, self.RECT[3] - 12), 2)
