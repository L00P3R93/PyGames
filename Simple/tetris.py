...
def main():
    ...
    while True:
        if random.randint(0, 1) == 0:
            pygame.mixer.music.load('../res/tetrisb.mid')
        else:
            pygame.mixer.music.load('../res/tetrisc.mid')
...
def showTextScreen(text):
    # This function displays large text in the center of screen until a key is pressed.
...
def checkForQuit():
    for event in pygame.event.get(QUIT):  # get all the QUIT events
        terminate()  # terminate if any QUIT are present
...
def addToBoard(board, piece):
    # fil in the baord based on piece's location, shape and rotation
...
def removeCompleteLines(board):
    # Remove any completes lines on the board, move everything above them down
    ...
    while y >= 0:
        if isCompleteLine(board, y):
            ...
            y -= 1  # move on to check row up
    return numLinesRemoved


def convertToPixelCoords(boxx, boxy):
    # Convert the given xy coordintaes of the board to xy
