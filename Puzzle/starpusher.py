...
# Total width and height of each title in pixels
TILEWIDTH = 50
TILEHEIGHT = 85
TILEFLOORHEIGHT = 40
...
# The percentage of outdoor tiles that have additional decoration
# on them, such as a tree or rock
...
def main():
    ...
    # A global dict that will contain all the Pygame
    ...
    # Read in the levels from text file.
        ...
        for event in pygame.event.get():
            if event.type == QUIT:
                ...
                # set the camera move mode
                    ...
                    return 'reset'  # Resets the level
                elif event.key == K_p:
                    # Change player image to the next one.
                    currentImage += 1
...
def makeMove(mapObj, gameStateObj, playerMoveTo):
    ...
    player to make the given move. If it is, then change the player's pisition
    (and the position of any pushed star). If not, do nothing.

    Returns True if the player moved, otherwise False."""

    # Make sure the player move in the direction they want.
    playerx, playery = gameStateObj['player']

    stars = gameStateObj['stars']

    # Handling each of the directions .
    # Adding or Subtracting 1 to the x/y coordinates. We can simplify
    # it by using the xOffset and yOffset variables
    if playerMoveTo == UP:
        xOffset = 0
        yOffset = -1
    ...
    if isWall(mapObj, playerx + xOffset, playery + yOffset):
        ...
        if (playerx + xOffset, playery + yOffset) in stars:
            # There is a star in the way, see if play can push it.
...
def readLevelsFile(filename):
    ...
    levels = []  # Contains a list of level objects
    levelNum = 0
    ...
    for lineNum in range(len(content)):
        ...
        if line != '':
            ...
            # A blank line indicates the end of a levels map in the file.
            ...
            assert startx is not None and starty is not None, 'Level %s (around line %s) in %s is missing "@" or "+" to mark the start point.' % (levelNum + 1, lineNum, filename)
            ...
            # Create level object and start game state object.
...
def drawMap(mapObj, gameStateObj, goals):
    ...
    This function does not call pygame.display.update(), nor does it
    draw "Level" and "Steps" text in the corner.

    mapSurf will be the single Surface object that the tiles are drawn on,
    so that it is easy to position the entire map on the DISPLAYSURF Surface object.
    First, the width and height must be calculated."""
    mapSurfWidth = len(mapObj) * TILEWIDTH
    mapSurfHeight = (len(mapObj[0]) - 1) * TILEFLOORHEIGHT + TILEHEIGHT
    mapSurf = pygame.Surface((mapSurfWidth, mapSurfHeight))
    mapSurf.fill(BGCOLOR)  # starting with a blnak color on the surface
    ...
    for x in range(len(mapObj)):
        for y in range(len(mapObj[x])):
            ...
            if mapObj[x][y] in OUTSIDEDECOMAPPING:
                ...
                if (x, y) in goals:
                    # A goal and star are on this space, draw first
            ...
            # Lastly, draw the player on the board
