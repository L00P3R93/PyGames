# Splash (a Flood-It clone)
# Try to make the entire field a single color

import random, sys, webbrowser, copy, pygame
from pygame.locals import *

# There are different boxsizes, number of boxes, and life
# depending on the "board size" setting selected.
SMALLBOXSIZE = 60
MEDIUMBOXSIZE = 20
LARGEBOXSIZE = 11
...
# the first color in each scheme is the background color, the next six are palette colors.
...
def main():
    ...
    # Load images
    LOGOIMAGE = pygame.image.load('../res/inkspilllogo.png')
    SPOTIMAGE = pygame.image.load('../res/inkspillspot.png')
    SETTINGSIMAGE = pygame.image.load('../res/inkspillsettings.png')
    SETTINGSBUTTONIMAGE = pygame.image.load('../res/inkspillsettingsbutton.png')
    RESETBUTTONIMAGE = pygame.image.load('../res/inkspillresetbutton.png')
    ...
    while True:
        ...
        for event in pygame.event.get():
            if event.type == MOUSEBUTTONUP:
                ...
                if pygame.Rect(WINDOWWIDTH - SETTINGSBUTTONIMAGE.get_width(), WINDOWHEIGHT - SETTINGSBUTTONIMAGE.get_height(), SETTINGSBUTTONIMAGE.get_width(), SETTINGSBUTTONIMAGE.get_height()).collidepoint(mousex, mousey):
                    resetGame = showSettingsScreen()  # Clicked on Settings button
                elif pygame.Rect(WINDOWWIDTH - RESETBUTTONIMAGE.get_width(), WINDOWHEIGHT - SETTINGSBUTTONIMAGE.get_height() - RESETBUTTONIMAGE.get_height(), RESETBUTTONIMAGE.get_width(), RESETBUTTONIMAGE.get_height()).collidepoint(mousex, mousey):
                    resetGame = True  # Clicked on Reset Button
                else:
                    # Check if palette button was clicked
                    paletteClicked = getColorOfPaletteAt(mousex, mousey)

        if paletteClicked is not None and paletteClicked != lastPaletteClicked:
            # A palette button was clicked that is different from the last
            # palette button cliked. This check prevents the player from clicking the same palette twice.
            ...
            if hasWon(mainBoard):
                for i in range(4):  # flash border 4 times
                    flashBorderAnimtation(WHITE, mainBoard)
                resetGame = True
                ...
                for i in range(4):
                    flashBorderAnimtation(BLACK, mainBoard)
                resetGame = True
                pygame.time.wait(2000)  # pause so the player can suffer in defeat
...
def checkForQuit():
    """Terminates the program if there are any QUIT or ESC events"""
...
def showSettingsScreen():
    ...
    the pixel coordinates from there."""
    ...
    while True:
        ...
        for event in pygame.event.get():
            if event.type == QUIT:
                ...
                if pygame.Rect(74, 16, 11, 30).collidepoint(mousex, mousey):
                    difficulty = EASY
                ...
                if pygame.Rect(63, 156, 84, 31).collidepoint(mousex, mousey):
                    ...
                    webbrowser.open('http://inventwithpyhton.com')
                    # opens a web browser
                elif pygame.Rect(178, 418, 215, 34).collidepoint(mousex, mousey):
                    # cliked on the "back to game" button
                    return not (origDifficulty == difficulty and origBoxSize == boxSize)
...
def flashBorderAnimtation(color, board, animationSpedd=30):
    origSurf = DISPLAYSURF.copy()
    flashSurf = pygame.Surface(DISPLAYSURF.get_size())
    flashSurf = flashSurf.convert_alpha()
    for start, end, step in ((0, 256, 1), (255, 0, -1)):
        """the first iteration on the outer loop will set the inner loop to
        have transparency go from 0 to 255, the second iteration will have it 
        go from 255 to 0. This is the "flash" """
        for transparency in range(start, end, animationSpedd * step):
...
def floodAnimation(board, paletteClicked, animationSpeed=25):
    ...
    for transparency in range(0, 255, animationSpeed):
        # The new board slowly becomes opaque over the original board
        drawBoard(origBoard)
...
def generateRandomBoard(width, height, difficulty=MEDIUM):
    ...
    for i in range(boxesToChange):
        # Randomly choose a boz whose color to copy
...
def drawLogoAndButtons():
    # draw the Ink Spill logo and Settings and Rest buttons.
...
def drawBoard(board, transparency=255):
    """The colored squares are drawn to a temporary surface which is then drawn
    to the DISPLAYSURF surface. This is done so we can draw the squares with
    transparency on top of DISPLAYSURF as it currently is."""
...
def drawPalettes():
    """Draw the six color palettes at bottom of the screen."""
